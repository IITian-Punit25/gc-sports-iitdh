...
export default function LivePage() {
    ...
    const liveMatches = results.filter(r => r.liveLink && r.liveLink.length > 0);
        ...
        <div className="min-h-screen bg-background text-foreground">
            ...
            <main className="max-w-7xl mx-auto px-4 py-12">
                <div className="text-center mb-16">
                    ...
                    <h1 className="text-4xl md:text-5xl font-black mb-6">Live Matches</h1>
                    <p className="text-slate-400 text-lg max-w-2xl mx-auto">
                        Watch live streams of ongoing matches and catch up on the action.
                    </p>
                </div>
                ...
                {loading ? (
                    ...
                    <div className="text-center py-20 bg-white/5 rounded-3xl border border-white/10">
                        ...
                        <p className="text-slate-400">There are no matches currently being streamed.</p>
                    </div>
                ) : (
                        ...
                        {liveMatches.map((match) => (
                            <div key={match.id} className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl overflow-hidden hover:border-primary/50 transition-all group">
                                <div className="aspect-video bg-black/50 relative flex items-center justify-center group-hover:bg-black/40 transition-colors">
                                    <div className="w-16 h-16 bg-red-600 rounded-full flex items-center justify-center shadow-lg shadow-red-600/20 group-hover:scale-110 transition-transform cursor-pointer">
                                        <Play className="h-8 w-8 text-white ml-1" />
                                    </div>
                                    <div className="absolute top-4 left-4 bg-red-600 text-white text-xs font-bold px-2 py-1 rounded flex items-center">
